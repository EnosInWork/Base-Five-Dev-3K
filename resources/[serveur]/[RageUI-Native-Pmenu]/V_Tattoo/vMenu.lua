...
local function debugPrint(text)
    if vMenu.debug then
        Citizen.Trace('[vMenu] '..tostring(text))
    end
end
    ...
    if menus[currentMenu] then
		...
		elseif menus[currentMenu].titleFont == "~sprite~" then
            ...
            if HasStreamedTextureDictLoaded("CommonMenu") then
                SetUiLayer(0)
                DrawSprite("CommonMenu", "interaction_bgd", x, y, menuWidth, titleHeight, 0.0, 255, 255, 255, 200, 0)
            end
...
-- API
function vMenu.CreateMenu(id, title, closeCallback)
    -- Default settings
    menus[id] = { }
    ...
    menus[id].buttonPressedSound = { name = "SELECT", set = "HUD_FRONTEND_CLOTHESSHOP_SOUNDSET" } --https://pastebin.com/0neZdsZ5
...
function vMenu.CreateSubMenu(id, parent, subTitle, closeCallback)
    if menus[parent] then
        vMenu.CreateMenu(id, menus[parent].title)
...
function vMenu.CurrentMenu()
    return currentMenu
end

function vMenu.MenuTable()
    return menus
end


function vMenu.OpenMenu(id)
    if id and menus[id] then
        ...
        debugPrint('Impossible d\'ouvrir'..tostring(id)..' ~r~C\'est fermer')
    end
end


function vMenu.IsMenuOpened(id)
    return isMenuVisible(id)
end


function vMenu.IsAnyMenuOpened()
...
function vMenu.IsAnyMenuWithTitleOpened(subtitle)
...
function vMenu.IsMenuAboutToBeClosed()
    if menus[currentMenu] then
...
function vMenu.IsThisMenuAboutToBeClosed(id)
    if menus[id] then
...
function vMenu.Button(text, subText)
    ...
    if menus[currentMenu] then
        ...
        if isCurrent then
			...
			elseif currentKey == keys.left or currentKey == keys.right then
				if text ~= "" then PlaySoundFrontend(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_CLOTHESSHOP_SOUNDSET", true) end
			end
        end
...
function vMenu.DisabledButton(text, subText)
    local buttonText = text
    if subText then
        buttonText = '{ '..tostring(buttonText)..', '..tostring(subText)..' }'
    end
...
function vMenu.SpriteButton(text, textDict, sprite, focusSprite)
    local buttonText = text
    ...
    if menus[currentMenu] then
        ...
        if isCurrent then
            if currentKey == keys.select then
                ...
                return true, isCurrent
            elseif currentKey == keys.left or currentKey == keys.right then
                PlaySoundFrontend(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_CLOTHESSHOP_SOUNDSET", true)
            end
        end
...
function vMenu.SpriteMenuButton(text, textDict, sprite, focusSprite, id)
    if menus[id] then
        local clicked, hovered = vMenu.SpriteButton(text, textDict, sprite, focusSprite)
        if clicked then
            setMenuVisible(currentMenu, false)
...
function vMenu.ComboBox(text, items, currentIndex, selectedIndex, callback, displaycb)
    ...
    if vMenu.Button(text, selectedItem) then
...
function vMenu.MenuButton(text, id, secondtext)
    if menus[id] then
		local clicked, hovered = vMenu.Button(text, (secondtext and secondtext or "→")) 
		if clicked then
            setMenuVisible(currentMenu, false)
            setMenuVisible(id, true, true)
...
function vMenu.SwitchMenu(id)
	setMenuVisible(currentMenu, false)
    setMenuVisible(id, true, true)
end

function vMenu.SetDescription(text)
	currentDesc = tostring(text)
end

function vMenu.CheckBox(text, bool, callback)
    --[[local checked = '~r~Off'
    if bool then
        checked = '~g~On'
    end]]
    ...
    if vMenu.SpriteButton(text, "commonmenu", sprite, focusSprite) then
        bool = not bool
    ...
    if isMenuVisible(currentMenu) and not IsPauseMenuActive() then
        ...
        if menus[currentMenu].aboutToBeClosed then
            ...
            if IsDisabledControlJustPressed(0, keys.down) or IsDisabledControlJustPressed(0, keys.mdown) then
                PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_CLOTHESSHOP_SOUNDSET", true)
            ...
            elseif IsDisabledControlJustPressed(0, keys.back) then
                if menus[menus[currentMenu].previousMenu] then
                    PlaySoundFrontend(-1, "BACK", "HUD_FRONTEND_CLOTHESSHOP_SOUNDSET", true)
...
function vMenu.SetMenuWidth(id, width)
    setMenuProperty(id, 'width', width)
end


function vMenu.SetMenuX(id, x)
    setMenuProperty(id, 'x', x)
end


function vMenu.SetMenuY(id, y)
    setMenuProperty(id, 'y', y)
end


function vMenu.SetMenuMaxOptionCountOnScreen(id, count)
    setMenuProperty(id, 'maxOptionCount', count)
end


function vMenu.SetTitleColor(id, r, g, b, a)
    setMenuProperty(id, 'titleColor', { r, g, b, a or  menus[id].titleColor[4] })
end


function vMenu.SetTitleBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, 'titleBackgroundColor', { r, g, b, a or menus[id].titleBackgroundColor[4] })
end


function vMenu.UseSpriteAsBackground(id, textDict, sprite, r, g, b, a, stillDrawText)
