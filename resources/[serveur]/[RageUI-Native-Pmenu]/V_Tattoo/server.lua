...
ESX.RegisterServerCallback('SmallTattoos:PurchaseTattoo', function(source, cb, tattooList, price, tattoo, tattooName)
	...
	if xPlayer.getMoney() >= price then
		...
		TriggerClientEvent('esx:showNotification', source, "Tu viens de te faire tatouer ~b~" .. tattooName .. "~s~ pour ~g~$" .. price)
		cb(true)
	else
		TriggerClientEvent('esx:showNotification', source, "Tu n\'as pas l'argent pour te payer ce genre de tattoo")
		cb(false)
	end
end)
