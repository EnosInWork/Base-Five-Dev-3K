...
function IsMenuOpen()
	return (vMenu.IsMenuOpened('tattoo') or string.find(tostring(vMenu.CurrentMenu() or ""), "ZONE_"))	
end
...
function RemoveTattoo(name, label)
	...
	ESX.ShowNotification("Tu as enlever le tattoo ~b~" .. GetLabelText(label) .. "~s~")
end

function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "Ouvrir le tattoo shop", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Changer de point de vue", 21, "Changer l'opacité", {90, 89}, "Acheter/enlever un tattoo", 191)
		scaleString = sType
	end
end

Citizen.CreateThread(function()
	vMenu.CreateMenu("tattoo", "Tattoo Shop", function()
        return CloseTattooShop()
    end)
    vMenu.SetSubTitle('tattoo', "Categories")
	
	for k, v in ipairs(Config.TattooCats) do
		vMenu.CreateSubMenu(v[1], "tattoo", v[2])
		vMenu.SetSubTitle(v[1], v[2])
	end
    ...
    while true do 
        ...
        if vMenu.IsMenuOpened('tattoo') then
			CanSleep = false
            for k, v in ipairs(Config.TattooCats) do
				vMenu.MenuButton(v[2], v[1])
			end
			ClearPedDecorations(PlayerPedId())
			for k,v in pairs(currentTattoos) do
				if v.Count ~= nil then
		...
		for k, v in ipairs(Config.TattooCats) do
			if vMenu.IsMenuOpened(v[1]) then
				CanSleep = false
					...
					if tattoo.Zone == v[1] then
						if GetEntityModel(PlayerPedId()) == `mp_m_freemode_01` then
							if tattoo.HashNameMale ~= '' then
								...
								if found then
									local clicked, hovered = vMenu.SpriteButton(GetLabelText(tattoo.Name), "commonmenu", "shop_tattoos_icon_a", "shop_tattoos_icon_b")
									if clicked then
										RemoveTattoo(tattoo.HashNameMale, tattoo.Name)
									end
								else
									local price = math.ceil(tattoo.Price / 20) == 0 and 100 or math.ceil(tattoo.Price / 20)
									local clicked, hovered = vMenu.Button(GetLabelText(tattoo.Name), "~HUD_COLOUR_GREENDARK~$" .. price)
									if clicked then
