          ...
          function(data, menu)
            ...
            if amount == nil then
              ...
              if closestPlayer == -1 or closestDistance > 3.0 then
                ...
                ESX.ShowNotification('Vous avez fait une facture de ~g~'..amount..'$')
              end
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'ambulance_wear' then
			...
			ESX.ShowNotification("~y~~h~Numpad 5 pour lancer les mission PNJ.")
        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'vehicle_spawner',
      {
        ...
        elements = {
		  ...
		  {label = 'Dodge Charger EMS',  value = 'dodgeems'},
        },
      },
      function(data, menu)
        menu.close()
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlPressed(0,  Keys['N5']) and (GetGameTimer() - GUI.Time) > 150 then -------------------------------------------------------------
...
function startRagdollWorkaround()
  ...
  Citizen.CreateThread(function()
      while work do
          Citizen.Wait(0)
          drawLoadingText('POUR ARRANGER TON PERSONNAGE ! TENIR VOTRE TÉLÉPHONE', 255,255,255,255)
