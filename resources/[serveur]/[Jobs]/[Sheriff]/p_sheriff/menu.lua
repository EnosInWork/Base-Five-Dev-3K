		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				if data.current.value ~= nil then
					...
					TriggerServerEvent('esx_sheriffjob:confiscatePlayerItem', GetPlayerServerId(player), itemType, itemName, amount)
...
local GaragesheriffMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Garage sheriff" },
	Data = { currentMenu = "Garage sheriff", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			...
			if btn == "sheriff Cruiser" then
				SetVehicleWindowTint(vehicle, 5)
				ESX.ShowNotification('Vous avez sorti un vehicule de sheriff')
				Citizen.Wait(1)
				spawnCar("sheriff")
			elseif btn == "Buffalo de sheriff" then	
				ESX.ShowNotification('Vous avez sorti un vehicule de sheriff')
				Citizen.Wait(1)
				spawnCar("sheriff2")
			elseif btn == "Interceptor" then	
				...
				Citizen.Wait(1)
				spawnCar("sheriff3")
			elseif btn == "Blinder sheriff" then
				ESX.ShowNotification('Vous avez sorti un vehicule de sheriff')
				Citizen.Wait(1)
				spawnCar("riot")
	...
	Menu = {
		["Garage sheriff"] = {
			b = {
				{name = "sheriff Cruiser", ask = ">", askX = true},
				{name = "Buffalo de sheriff", ask = ">", askX = true},
				{name = "Interceptor", ask = ">", askX = true},
				{name = "Blinder sheriff", ask = ">", askX = true},
...
function spawnCar(car)
    ...
    local vehicle = CreateVehicle(car, 453.953, -984.007, 25.72, 359.484, true, false)
...
local mobgarbageMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Interaction Sheriff" },
	Data = { currentMenu = "Interaction sheriff", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
			local slide = btn.slidenum
...
elseif btn == "Amendes" then
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		CloseMenu()
	TriggerServerEvent('esx_sheriffjob:message', GetPlayerServerId(closestPlayer), ('being_searched'))
...
---------------- FIN MENU F6 garbage ---------------
local ArmeMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Armes sheriff" },
	...
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			...
			elseif btn == "Tazer Volts x26 - sheriff" then	
				TriggerServerEvent('esx_sheriffjob:AddTazer') 
			elseif btn == "Beretta 92 - sheriff" then	
				TriggerServerEvent('esx_sheriffjob:AddPistol')
			elseif btn == "Glock 17 - sheriff" then
				TriggerServerEvent('esx_sheriffjob:AddComPistol')
			elseif btn == "Fusil à pompe - sheriff" then
				TriggerServerEvent('esx_sheriffjob:AddPump')
			elseif btn == "9mm - sheriff" then
	...
	Menu = {
		["Armes de services"] = {
			b = {
				...
				{name = "Tazer Volts x26 - sheriff", ask = ">", askX = true},
				{name = "Beretta 92 - sheriff", ask = ">", askX = true},
				{name = "Glock 17 - sheriff", ask = ">", askX = true},
				{name = "Fusil à pompe - sheriff", ask = ">", askX = true},
		--		{name = "9mm - sheriff", ask = ">", askX = true}
			}
		}
	}
}
	...
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			...
			if btn == "Reprendre vos affaires" then
				...
				TriggerEvent('esx_sheriffjob:offDuty')
                ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
                    TriggerEvent('skinchanger:loadSkin', skin)
				end)
				SetPedArmour(playerPed, 0)			
			elseif btn == "Tenue sheriff" then
				TriggerServerEvent("player:serviceOn", "sheriff")
				TriggerEvent('esx_sheriffjob:onDuty')
	...
	Menu = {
		["Vos tenues"] = {
			b = {
				...
				{name = "----------------------------------------"},
				{name = "Tenue sheriff", ask = ">", askX = true},
...
local cloak = {
    {x = -439.27, y = 6004.53, z = 31.71},
}

local garage = {
 --   {x = 456.836, y = -991.396, z = 25.72}
}

local arm = {
    {x = -441.998, y = 5987.4218, z = 31.71},
}

local boss = {
    {x = 470.521, y = -1006.465, z = 30.69}
}

local del = {
    {x = -449.67, y = 6032.465, z = 31.34}
}

Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
		if IsControlJustPressed(1,167) and PlayerData.job and PlayerData.job.name == 'sheriff' then
		...
		for k in pairs(arm) do
            ...
            if dist <= 2.2 and PlayerData.job and  PlayerData.job.name == 'sheriff' then
		...
		for k in pairs(del) do
            ...
            if dist <= 2.2 and PlayerData.job and PlayerData.job.name == 'sheriff' and IsPedInAnyVehicle(PlayerPedId(-1), false) then
		...
		for k in pairs(boss) do
            ...
            if dist <= 2.2 and PlayerData.job and PlayerData.job.name == 'sheriff' and PlayerData.job.grade_name == 'boss' then
