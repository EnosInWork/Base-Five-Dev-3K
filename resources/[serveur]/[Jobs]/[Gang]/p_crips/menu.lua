...
local GaragecripsMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Garage crips" },
	Data = { currentMenu = "Garage crips", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			...
			if btn == "crips Cruiser" then
				SetVehicleWindowTint(vehicle, 5)
				ESX.ShowNotification('Vous avez sorti un vehicule de crips')
				Citizen.Wait(1)
				spawnCar("crips")
...
function spawnCar(car)
    ...
    local vehicle = CreateVehicle(car, 1434.04, -1509.011, 62.00, 101.65, true, false)
    SetPedIntoVehicle(PlayerPedId(), vehicle, -1)
    
    SetEntityAsNoLongerNeeded(vehicle)
	SetModelAsNoLongerNeeded(vehicleName)
	SetVehicleWindowTint(vehicle, 3)
	SetVehicleNumberPlateText(vehicle, "crips")
	local color = GetIsVehiclePrimaryColourCustom(vehicle)
	SetVehicleCustomPrimaryColour(vehicle, 0, 81, 255)
	SetVehicleCustomSecondaryColour(vehicle, 0, 81, 255)    
end
...
local mobgarbageMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Interaction crips" },
	Data = { currentMenu = "Interaction crips", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
			local slide = btn.slidenum
...
elseif btn == "Fouiller" then
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		CloseMenu()
	TriggerServerEvent('esx_cripsjob:message', GetPlayerServerId(closestPlayer), ('being_searched'))
...
---------------- FIN MENU F6 garbage ---------------
local ArmeMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Armes crips" },
	Data = { currentMenu = "Armes crips", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
			local slide = btn.slidenum
			...
			elseif btn == "9mm" then	
				TriggerServerEvent('esx_cripsjob:AddPistol')
			elseif btn == "Couteau" then
				TriggerServerEvent('esx_cripsjob:AddComPistol')
			elseif btn == "Club de Golf" then
				TriggerServerEvent('esx_cripsjob:AddPump')
...
local blips = {
	-- Example {title="", colour=, id=, x=, y=, z=},
	 ...
	 {title="~b~Quartier~s~ | Crips", colour=38, id=378, x = 1436.986, y = -1491.701, z = 50.0},
...
local garage = {
    {x = 1435.38, y = -1499.60, z = 63.22}
}

local arm = {
    {x = 1443.643, y = -1490.96, z = 66.62},
}

local boss = {
    {x = 1442.01, y = -1494.96, z = 63.70}
}

local del = {
    {x = 1432.79, y = -1509.735, z = 61.87}
}

Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
		if IsControlJustPressed(1,167) and PlayerData.job and PlayerData.job.name == 'crips' then
		...
		for k in pairs(arm) do
            ...
            if dist <= 2.2 and PlayerData.job and  PlayerData.job.name == 'crips' then
		...
		for k in pairs(del) do
            ...
            if dist <= 2.2 and PlayerData.job and PlayerData.job.name == 'crips' and IsPedInAnyVehicle(PlayerPedId(-1), false) then
		...
		for k in pairs(boss) do
            ...
            if dist <= 2.2 and PlayerData.job and PlayerData.job.name == 'crips' and PlayerData.job.grade_name == 'boss' then
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "s_m_y_armymech_01", 1436.22, -1499.81, 62.22, 70.25, false, true)
