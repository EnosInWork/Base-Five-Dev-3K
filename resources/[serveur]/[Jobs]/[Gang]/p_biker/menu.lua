...
local GaragebikerMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Garage biker" },
	Data = { currentMenu = "Garage biker", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			...
			if btn == "biker Cruiser" then
				SetVehicleWindowTint(vehicle, 5)
				ESX.ShowNotification('Vous avez sorti un vehicule de biker')
				Citizen.Wait(1)
				spawnCar("biker")
			elseif btn == "Gargoyle" then
				ESX.ShowNotification('Vous avez sorti un vehicule de Biker')
				Citizen.Wait(1)
				spawnCar("Gargoyle")
			elseif btn == "Daemon" then
				ESX.ShowNotification('Vous avez sorti un vehicule de Biker')
				Citizen.Wait(1)
				spawnCar("Daemon")
		end
	...
	Menu = {
		["Garage biker"] = {
			b = {
				{name = "Daemon", ask = ">", askX = true},
				{name = "Gargoyle", ask = ">", askX = true}
			}
		}
	}
}
...
function spawnCar(car)
	...
	SetVehicleNumberPlateText(vehicle, "Xx")
...
local mobgarbageMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Interaction biker" },
	Data = { currentMenu = "Interaction biker", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
			local slide = btn.slidenum
...
elseif btn == "Fouiller" then
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		CloseMenu()
	TriggerServerEvent('esx_bikerjob:message', GetPlayerServerId(closestPlayer), ('being_searched'))
...
---------------- FIN MENU F6 garbage ---------------
local ArmeMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Armes Biker" },
	Data = { currentMenu = "Armes Biker", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
			local slide = btn.slidenum
			...
			elseif btn == "9mm" then	
				TriggerServerEvent('esx_bikerjob:AddPistol')
			elseif btn == "Couteau" then
				TriggerServerEvent('esx_bikerjob:AddComPistol')
			elseif btn == "Club de Golf" then
				TriggerServerEvent('esx_bikerjob:AddPump')
...
local garage = {
    {x = 989.77, y = -110.07, z = 74.07}
}

local arm = {
    {x = 973.07, y = -97.356, z = 74.87},
}

local boss = {
    {x = 977.081, y = -104.158, z = 74.84}
}

local del = {
    {x = 981.33, y = -114.249, z = 74.07}
}

Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
		if IsControlJustPressed(1,167) and PlayerData.job and PlayerData.job.name == 'biker' then
		...
		for k in pairs(arm) do
            ...
            if dist <= 2.2 and PlayerData.job and  PlayerData.job.name == 'biker' then
		...
		for k in pairs(del) do
            ...
            if dist <= 2.2 and PlayerData.job and PlayerData.job.name == 'biker' and IsPedInAnyVehicle(PlayerPedId(-1), false) then
		...
		for k in pairs(boss) do
            ...
            if dist <= 2.2 and PlayerData.job and PlayerData.job.name == 'biker' and PlayerData.job.grade_name == 'boss' then
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "s_m_y_armymech_01", 990.24, -110.297, 73.06, 63.25, false, true)
