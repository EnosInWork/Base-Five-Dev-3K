...
local GarageVagosMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Garage Vagos" },
	Data = { currentMenu = "Garage Vagos", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			...
			if btn == "vagos Cruiser" then
				SetVehicleWindowTint(vehicle, 5)
				ESX.ShowNotification('Vous avez sorti un vehicule de vagos')
				Citizen.Wait(1)
				spawnCar("vagos")
			elseif btn == "Buffalo de vagos" then	
				ESX.ShowNotification('Vous avez sorti un vehicule de vagos')
				Citizen.Wait(1)
				spawnCar("vagos2")
			elseif btn == "Interceptor" then	
				...
				Citizen.Wait(1)
				spawnCar("vagos3")
			elseif btn == "Blinder Vagos" then
				ESX.ShowNotification('Vous avez sorti un vehicule de vagos')
				Citizen.Wait(1)
				spawnCar("riot")
			elseif btn == "Elegy" then
				ESX.ShowNotification('Vous avez sorti un vehicule de vagos')
				Citizen.Wait(1)
				spawnCar("Elegy")
			elseif btn == "Sultan" then
	...
	Menu = {
		["Garage Vagos"] = {
			b = {
				{name = "Sultan", ask = ">", askX = true},
				{name = "Elegy", ask = ">", askX = true}
			}
		}
	}
}
...
function spawnCar(car)
    ...
    local vehicle = CreateVehicle(car, 332.043, -2032.25, 21.13, 142.583, true, false)
...
local mobgarbageMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Interaction Vagos" },
	Data = { currentMenu = "Interaction Vagos", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
			local slide = btn.slidenum
...
elseif btn == "Amendes" then
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		CloseMenu()
	TriggerServerEvent('esx_vagosjob:message', GetPlayerServerId(closestPlayer), ('being_searched'))
...
---------------- FIN MENU F6 garbage ---------------
local ArmeMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Armes Vagos" },
	Data = { currentMenu = "Armes Vagos", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
			local slide = btn.slidenum
			...
			elseif btn == "Tazer Volts x26 - Vagos" then	
				TriggerServerEvent('esx_vagosjob:AddTazer') 
			elseif btn == "9mm" then	
				TriggerServerEvent('esx_vagosjob:AddPistol')
			elseif btn == "Couteau" then
				TriggerServerEvent('esx_vagosjob:AddComPistol')
			elseif btn == "Club de Golf" then
				TriggerServerEvent('esx_vagosjob:AddPump')
	...
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			...
			if btn == "Reprendre vos affaires" then
				TriggerServerEvent("player:serviceOff", "vagos")
				TriggerEvent('esx_vagosjob:offDuty')
                ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
                    TriggerEvent('skinchanger:loadSkin', skin)
				end)
				SetPedArmour(playerPed, 0)			
			elseif btn == "Tenue Vagos" then
				TriggerServerEvent("player:serviceOn", "vagos")
				TriggerEvent('esx_vagosjob:onDuty')
	...
	Menu = {
		["Vos tenues"] = {
			b = {
				...
				{name = "----------------------------------------"},
				{name = "Tenue Vagos", ask = ">", askX = true},
...
local blips = {
	-- Example {title="", colour=, id=, x=, y=, z=},
	 ...
	 {title="~y~Quartier~s~ | Vagos", colour=5, id=378, x = 361.22, y = -2041.49, z = 10.632},
...
local garage = {
    {x = 337.453, y = -2036.39, z = 21.33}
}

local arm = {
    {x = 365.242, y = -2041.427, z = 22.394},
}

local boss = {
    {x = 370.369, y = -2042.832, z = 22.39}
}

local del = {
    {x = 332.423, y = -2031.661, z = 21.18}
}

Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
		if IsControlJustPressed(1,167) and PlayerData.job and PlayerData.job.name == 'vagos' then
		...
		for k in pairs(arm) do
            ...
            if dist <= 2.2 and PlayerData.job and  PlayerData.job.name == 'vagos' then
		...
		for k in pairs(del) do
            ...
            if dist <= 2.2 and PlayerData.job and PlayerData.job.name == 'vagos' and IsPedInAnyVehicle(PlayerPedId(-1), false) then
		...
		for k in pairs(boss) do
            ...
            if dist <= 2.2 and PlayerData.job and PlayerData.job.name == 'vagos' and PlayerData.job.grade_name == 'boss' then
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "s_m_y_armymech_01", 338.138, -2035.711, 20.41, 134.408, false, true)
