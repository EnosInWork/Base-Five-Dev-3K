...
RegisterServerEvent('cartel:PriseEtFinservice')
AddEventHandler('cartel:PriseEtFinservice', function(PriseOuFin)
	...
	for i = 1, #xPlayers, 1 do
		...
		if thePlayer.job.name == 'cartel' then
			TriggerClientEvent('cartel:InfoService', xPlayers[i], _raison, name)
		end
	end
end)


RegisterServerEvent('esx_carteljob:AddPistol')
AddEventHandler('esx_carteljob:AddPistol', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_carteljob:AddComPistol')
AddEventHandler('esx_carteljob:AddComPistol', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_carteljob:AddPump')
AddEventHandler('esx_carteljob:AddPump', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
TriggerEvent('esx_phone:registerCallback', function(source, phoneNumber, message, anon)
	...
	if phoneNumber == 'cartel' then
		 for i=1, #xPlayers, 1 do
		 	local xPlayer2 = ESX.GetPlayerFromId(xPlayers[i])
			if xPlayer2.job.name == 'cartel' then
				TriggerEvent('esx_phone:getDistpatchRequestId', function(requestId)
					TriggerClientEvent('esx_phone:onMessage', xPlayer2.source, xPlayer.get('phoneNumber'), message, xPlayer.get('coords'), anon, _('alert_cartel'), requestId)
				end)
			end
		end
	end
