...
local GaragecartelMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Garage cartel" },
	Data = { currentMenu = "Garage cartel", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			...
			if btn == "cartel Cruiser" then
				SetVehicleWindowTint(vehicle, 5)
				ESX.ShowNotification('Vous avez sorti un vehicule de cartel')
				Citizen.Wait(1)
				spawnCar("cartel")
			elseif btn == "Buffalo de cartel" then	
				ESX.ShowNotification('Vous avez sorti un vehicule de cartel')
				Citizen.Wait(1)
				spawnCar("cartel2")
			elseif btn == "Interceptor" then	
				...
				Citizen.Wait(1)
				spawnCar("cartel3")
			elseif btn == "Blinder cartel" then
				ESX.ShowNotification('Vous avez sorti un vehicule de cartel')
				Citizen.Wait(1)
				spawnCar("riot")
			elseif btn == "Sadler" then
				ESX.ShowNotification('Vous avez sorti un vehicule')
				Citizen.Wait(1)
				spawnCar("Sadler2")
			elseif btn == "Zion" then
				ESX.ShowNotification('Vous avez sorti un vehicule')
				Citizen.Wait(1)
				spawnCar("Zion")
		end
	...
	Menu = {
		["Garage cartel"] = {
			b = {
				{name = "Sadler", ask = ">", askX = true},
				{name = "Zion", ask = ">", askX = true}
			}
		}
	}
}
...
function spawnCar(car)
    ...
    local vehicle = CreateVehicle(car, -1894.372, 2047.359, 140.811, 245.35, true, false)
	...
	SetVehicleCustomPrimaryColour(vehicle, 1, 1, 1)
	SetVehicleCustomSecondaryColour(vehicle, 1, 1, 1)    
end
...
local mobgarbageMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Interaction cartel" },
	Data = { currentMenu = "Interaction cartel", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
			local slide = btn.slidenum
...
elseif btn == "Amendes" then
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		CloseMenu()
	TriggerServerEvent('esx_carteljob:message', GetPlayerServerId(closestPlayer), ('being_searched'))
...
---------------- FIN MENU F6 garbage ---------------
local ArmeMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Armes cartel" },
	Data = { currentMenu = "Armes cartel", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
			local slide = btn.slidenum
			...
			elseif btn == "Tazer Volts x26 - cartel" then	
				TriggerServerEvent('esx_carteljob:AddTazer') 
			elseif btn == "9mm" then	
				TriggerServerEvent('esx_carteljob:AddPistol')
			elseif btn == "Couteau" then
				TriggerServerEvent('esx_carteljob:AddComPistol')
			elseif btn == "Club de Golf" then
				TriggerServerEvent('esx_carteljob:AddPump')
	...
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			...
			if btn == "Reprendre vos affaires" then
				TriggerServerEvent("player:serviceOff", "cartel")
				TriggerEvent('esx_carteljob:offDuty')
                ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
                    TriggerEvent('skinchanger:loadSkin', skin)
				end)
				SetPedArmour(playerPed, 0)			
			elseif btn == "Tenue cartel" then
				TriggerServerEvent("player:serviceOn", "cartel")
				TriggerEvent('esx_carteljob:onDuty')
	...
	Menu = {
		["Vos tenues"] = {
			b = {
				...
				{name = "----------------------------------------"},
				{name = "Tenue cartel", ask = ">", askX = true},
...
local cloak = {
 --   {x = 473.849, y = -986.29, z = 25.73},
}

local garage = {
    {x = -1897.57, y = 2051.654, z = 140.831}
}

local arm = {
    {x = -1886.43, y = 2062.315, z = 140.98},
}

local boss = {
    {x = -1890.811, y = 2058.703, z = 140.98}
}

local del = {
    {x = -1889.562, y = 2045.53, z = 140.87}
}

Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
		if IsControlJustPressed(1,167) and PlayerData.job and PlayerData.job.name == 'cartel' then
		...
		for k in pairs(arm) do
            ...
            if dist <= 2.2 and PlayerData.job and  PlayerData.job.name == 'cartel' then
		...
		for k in pairs(del) do
            ...
            if dist <= 2.2 and PlayerData.job and PlayerData.job.name == 'cartel' and IsPedInAnyVehicle(PlayerPedId(-1), false) then
		...
		for k in pairs(boss) do
            ...
            if dist <= 2.2 and PlayerData.job and PlayerData.job.name == 'cartel' and PlayerData.job.grade_name == 'boss' then
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "s_m_y_armymech_01", -1897.33, 2052.24, 139.87, 157.44, false, true)
