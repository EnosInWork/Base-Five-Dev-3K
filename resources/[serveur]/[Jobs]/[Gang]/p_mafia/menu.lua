...
local GarageLSPDMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Garage LSPD" },
	...
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			...
			if btn == "mafia Cruiser" then
				SetVehicleWindowTint(vehicle, 5)
				ESX.ShowNotification('Vous avez sorti un vehicule de mafia')
				Citizen.Wait(1)
				spawnCar("mafia")
			elseif btn == "Buffalo de mafia" then	
				ESX.ShowNotification('Vous avez sorti un vehicule de mafia')
				Citizen.Wait(1)
				spawnCar("mafia2")
			...
			elseif btn == "Blinder LSPD" then
				...
				Citizen.Wait(1)
				spawnCar("riot")
			elseif btn == "Schafter" then
				ESX.ShowNotification('Vous avez sorti un vehicule de mafia')
				Citizen.Wait(1)
				spawnCar("schafter2")
			elseif btn == "Cheburek" then
	...
	Menu = {
		["Garage LSPD"] = {
			b = {
				{name = "Cheburek", ask = ">", askX = true},
				{name = "Schafter", ask = ">", askX = true}
			}
		}
	}
}
...
function spawnCar(car)
    ...
    local vehicle = CreateVehicle(car, 1360.07, 1155.60, 113.75, 106.973, true, false)
    SetPedIntoVehicle(PlayerPedId(), vehicle, -1)
    
    SetEntityAsNoLongerNeeded(vehicle)
	SetModelAsNoLongerNeeded(vehicleName)
	SetVehicleWindowTint(vehicle, 3)
	SetVehicleNumberPlateText(vehicle, "XXXX")
...
local mobgarbageMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Interaction mafia" },
	Data = { currentMenu = "Interaction mafia", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
			local slide = btn.slidenum
...
elseif btn == "Amendes" then
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		CloseMenu()
	TriggerServerEvent('esx_mafiajob:message', GetPlayerServerId(closestPlayer), ('being_searched'))
...
--------------------------------------------------------------------------------
	...
	Menu = {
		...
		["interaction_menottes"] = {
			b = {
				{name = "Menotter/Démenotter l'individu", ask = ">", askX = true},
...
---------------- FIN MENU F6 garbage ---------------
local ArmeMenu = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {0, 0, 0}, Title = "Armes Mafia" },
	Data = { currentMenu = "Armes Mafia", "Test" },
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
			local slide = btn.slidenum
			...
			elseif btn == "Tazer Volts x26 - Vagos" then	
				TriggerServerEvent('esx_mafiajob:AddTazer') 
			elseif btn == "9mm" then	
				TriggerServerEvent('esx_mafiajob:AddPistol')
			elseif btn == "Couteau" then
				TriggerServerEvent('esx_mafiajob:AddComPistol')
			elseif btn == "Club de Golf" then
				TriggerServerEvent('esx_mafiajob:AddPump')
	...
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			...
			if btn == "Reprendre vos affaires" then
				TriggerServerEvent("player:serviceOff", "mafia")
				TriggerEvent('esx_mafiajob:offDuty')
                ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
                    TriggerEvent('skinchanger:loadSkin', skin)
				end)
				SetPedArmour(playerPed, 0)			
			elseif btn == "Tenue LSPD" then
				TriggerServerEvent("player:serviceOn", "mafia")
				TriggerEvent('esx_mafiajob:onDuty')
...
local garage = {
    {x = 1363.016, y = 1158.50, z = 113.75}
}

local arm = {
    {x = 1404.192, y = 1150.077, z = 114.333},
}

local boss = {
    {x = 1394.095, y = 1160.54, z = 114.37}
}

local del = {
    {x = 1349.82, y = 1148.061, z = 113.758}
}

Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
		if IsControlJustPressed(1,167) and PlayerData.job and PlayerData.job.name == 'mafia' then
		...
		for k in pairs(arm) do
            ...
            if dist <= 2.2 and PlayerData.job and  PlayerData.job.name == 'mafia' then
		...
		for k in pairs(del) do
            ...
            if dist <= 2.2 and PlayerData.job and PlayerData.job.name == 'mafia' and IsPedInAnyVehicle(PlayerPedId(-1), false) then
		...
		for k in pairs(boss) do
            ...
            if dist <= 2.2 and PlayerData.job and PlayerData.job.name == 'mafia' and PlayerData.job.grade_name == 'boss' then
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "s_m_y_armymech_01", 1364.027, 1159.24, 112.75, 160.69, false, true)
