   ...
   --   table.insert(elements, {label = _U('fems_clothes_ems'), value = 'famb_wear'})
    ...
    ESX.UI.Menu.Open(
      ...
      if data.current.value == 'ambulance_wear' then
        ...
        ESX.TriggerServerCallback('skinchanger:getSkin', function(skin)
            if skin.sex == 0 then
              TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms.ambulance_wear.male)
            elseif skin.sex == 1 then
              TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms.ambulance_wear.female)
            end
          end)
