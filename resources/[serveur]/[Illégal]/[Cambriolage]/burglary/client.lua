		...
		if IsControlJustPressed(0, 51) and not onMission and IsPedInAnyVehicle(PlayerPedId()) then
			...
			if IsMissionVehicle(GetEntityModel(veh)) then
				...
				if time >= 0 and time <= TimeToSeconds(5, 30, 0) then
					...
					ShowMPMessage("Voleur", "trouve une maison a voler.", 3500)
...
CreateThread(function()
	while true do
		...
		if onMission then
			...
			-- draw info
			DrawTimerBar("Objets", #stolenItems, 2)
			DrawTimerBar("Jour", AddLeadingZero(time.hours) .. ":" .. AddLeadingZero(time.minutes), 1)
		end
	end
end)
		...
		if onMission then
			for k,door in pairs(doors) do
				...
				if Vdist(coords, door.coords.x, door.coords.y, door.coords.z) < 2 then
					DisplayHelpText("Appuyez sur le ~INPUT_CONTEXT~ pour entrer dans la maison.")
					...
					if IsControlJustPressed(0, 51) then
						...
						ShowSubtitle("Vous devez faire attention a l'intérieur de la maison!")
					end
				end
			end
		end
